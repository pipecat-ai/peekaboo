...
Where things stand on 2026-09-10: the app runs from a checkout on macOS
with every screen of the window working by mouse and by voice, speech on
the machine (Moonshine hears and is the wake word, Kokoro speaks), the
models and API keys chosen in Settings, six Pipecat workers on one bus
(four of them `LLMWorker`s), and the evals green. What remains is in the
M5 and M6 rows below: the Mac quiet rules, launch at login, a hotkey,
permission onboarding, and packaging, which was left for last on purpose.
